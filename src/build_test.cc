...
bool FakeCommandRunner::StartCommand(Edge* edge) {
  ...
  if (edge->rule().name() == "cat"  ||
    ...
    string content;
    string err;
    if (fs_->ReadFile(edge->inputs_[0]->path(), &content, &err) == 0)
...
"build in: dyndep | circ\n"
  ...
  EXPECT_FALSE(builder_.Build(&err));
  // Depending on how the pointers work out, we could have
...
"build out2: dyndep | out1.imp\n"
  ...
  EXPECT_EQ("", err);
  ASSERT_EQ(3u, command_runner_.commands_ran_.size());
  // Depending on how the pointers work out, the first two commands
  // may have run in either order.
...
"build out2 | out2.imp: dyndep | out1.imp\n"
  ...
  // During the build dd1 should be built and loaded, and then dd2
  // should be loaded when the builder recognizes that it is now ready.
  // If dd2 were loaded during RecomputeDirty executed after loading dd1
  // then the build will get stuck because the builder will never get a
  // chance to update the build plan from loading dd2.  This test case
  // verifies that each dyndep file is loaded and updates the build graph
  // independently.
...
TEST_F(BuildTest, DyndepTwoLevelIndirect) {
  // Verify that dyndep files can add implicit inputs to an edge that
...
"build out2 | out2.imp: dyndep | out1.imp\n"
  ...
  // During the build dd1 should be built and loaded, and then dd2
  // should be loaded when the builder recognizes that it is now ready.
  // When dd2 is loaded the builder should recognize that out2 needs to
  // be built even though it was originally clean without dyndep info.
